define(["template"], function(template) {
	...
	var api_host = "http://172.20.0.39:8080/dsp-op";
	var APIS = {
		login: {
	...
	var common = {
		...
		renderCreativeInfo: function(data) {
			...
			for (var key in data && data.data) {
				...
				if (data.data.adType == 3) {
					$("#bannerUrl").addClass("hidden");
					$("video").removeClass("hidden").attr("src",data.data.videoUrl);
				} else {
					$("#bannerUrl").attr("src", data.data.bannerUrl || data.data.iconUrl || data.data.picUrl);	
				}
	        }
			...
			*/
			conveterParamsToJson: function(paramsAndValues) {
			    ...
			    for (var i = 0; param != null && i < param.length; i++) {
			        ...
			        if (!isNaN(para[1]) && para[0] != "password" && para[0] != "authCode" && para[0] != "comment" && para[0] != "reason") {
			        	para[1] = para[1] * 1;
			        }
		...
		ajaxRequest: function(api, data, successCallback, beforeSend) {
			...
			var ajaxOptions = {
				...
				complete: function(content,type,status) {
	...
	return {
		urlTools:common.urlTools,
		handleAuthority:common.handleAuthority,
		ajaxRequest:common.ajaxRequest,
		buildPage:common.buildPage,
		APIS:APIS,
		msgs:common.msgs,
		getAuthorityAndSetLocation:common.getAuthorityAndSetLocation
	};
})
